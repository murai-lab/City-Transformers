...
class BaseSSL(nn.Module):
    ...
    IMAGENET_PATH = os.environ.get('IMAGENET_PATH', '/home/yzhang37/data/Imagenet/imagenet/')
    MVIT_PATH = os.environ.get('MVIT_PATH', '/home/yzhang37/data/data_mvit/')
    ...
    def __init__(self, hparams):
        ...
        elif hparams.data == 'mvit':
            print(f"MVIT_PATH = {self.MVIT_PATH}")
    ...
    def prepare_data(self):
        ...
        elif self.hparams.data == 'mvit':
            traindir = os.path.join(self.MVIT_PATH, 'train')
            valdir = os.path.join(self.MVIT_PATH, 'test')
            self.trainset = datasets.ImageFolder(traindir, transform=train_transform)
            self.testset = datasets.ImageFolder(valdir, transform=test_transform)
        else:
...
class SSLEval(BaseSSL):
    ...
    def step(self, batch):
        if self.hparams.problem == 'eval' and self.hparams.data == 'imagenet':
            batch[0] = batch[0] / 255.
        elif self.hparams.problem == 'eval' and self.hparams.data == 'mvit':
            batch[0] = batch[0] / 255.
        h, y = batch
