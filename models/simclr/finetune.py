...
from pick_models import ResNet50
...
parser = argparse.ArgumentParser(
        prog='Simlr fine-tune',
        description='Simlr fine-tune with Resnet50',
    )
...
# data
parser.add_argument("--dataset", default='abap', help='dataset', choices=['abap', 'gsv'])
...
# phase
parser.add_argument('--phase', default='classifier', help='training phase', choices=['classifier', 'fine-tune', 'eval'])
...
def main():
    ...
    if args.phase == 'classifier' or args.phase == 'fine-tune':
        ...
        if args.phase == 'classifier':
            model = ResNet50(args.numofcat, weighted=args.weighted, path=args.path, fixed=True, device=args.device)
        else:
            model = ResNet50(args.numofcat, weighted=args.weighted, path=args.path, fixed=False, device=args.device)
        ...
        if glob(f'{result_dir}.json'):
            ...
            if args.vis:
                ...
                vis_folder = ''
            dataset = ABAP(args.dataset, DATA_PATH, trainsize=1, testsize=args.testsize)
            dataset.prepare_data()
            # test_loader, _ = dataset .dataloaders() # train_info
            _, test_loader = dataset .dataloaders()
            model = ResNet50(args.numofcat, weighted=False, path=None, fixed=False)
